...
(function () {
  ...
  ctx = function (o) {
    ...
    this.enableMirroring =
      options.enableMirroring !== undefined ? options.enableMirroring : defaultOptions.enableMirroring;
  ...
  ctx.prototype.setAttrs = function (pen) {
    ...
    if (pen.type === 1) {
      pen.from.id && currentElement.setAttribute('from-id', pen.from.id);
      pen.to.id && currentElement.setAttribute('to-id', pen.to.id);
    }

    if (!pen.data) {
      return;
    }
    if (Array.isArray(pen.data)) {
      pen.data.forEach((attr) => {
        if (attr.key) {
          currentElement.setAttribute('data-' + attr.key, attr.value);
        }
      });
    } else if (typeof pen.data === 'object') {
      for (const key in pen.data) {
        if (key) {
          currentElement.setAttribute('data-' + key, pen.data[key]);
        }
      }
    }
  };
  ...
  ctx.prototype.ellipse = function (x, y, radiusX, radiusY, rotation, startAngle, endAngle, counterClockwise) {
    ...
    var endX =
        x + Math.cos(-rotation) * radiusX * Math.cos(endAngle) + Math.sin(-rotation) * radiusY * Math.sin(endAngle),
      endY =
        y - Math.sin(-rotation) * radiusX * Math.cos(endAngle) + Math.cos(-rotation) * radiusY * Math.sin(endAngle),
      startX =
        x + Math.cos(-rotation) * radiusX * Math.cos(startAngle) + Math.sin(-rotation) * radiusY * Math.sin(startAngle),
      startY =
        y - Math.sin(-rotation) * radiusX * Math.cos(startAngle) + Math.cos(-rotation) * radiusY * Math.sin(startAngle),
      sweepFlag = counterClockwise ? 0 : 1,
      largeArcFlag = 0,
      diff = endAngle - startAngle;
